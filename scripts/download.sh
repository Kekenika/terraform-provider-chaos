...
readonly RELEASE_URL="https://github.com/Kekenika/terraform-provider-chaos/releases"
...
download() {
	...
	if is_windows; then
		curl -sL -o "$temp_file" \
			"$RELEASE_URL/download/$VERSION/terraform-provider-chaos_$(uname -s)_$(uname -m).zip"
	else
		curl -sL -o "$temp_file" \
			"$RELEASE_URL/download/$VERSION/terraform-provider-chaos_$(uname -s)_$(uname -m).tar.gz"
	fi
}
...
cp "$TEMP_DIR/terraform-provider-slack" ./terraform-provider-chaos_${VERSION}
